...
if(isset($_POST['update']))
{
	...
	$query="UPDATE  `registration` SET regno=?,firstName=?,middleName=?,lastName=?,gender=?,contactno=?,updationDate=? WHERE email=?";
	$stmt = $mysqli->prepare($query);
	$rc=$stmt->bind_param('sssssisi',$regno,$fname,$mname,$lname,$gender,$contactno,$udate,$email);
	$stmt->execute();
...
// 		$contactno=$_POST['contact'];
// 		$udate = date('d-m-Y h:i:s', time());
// 		$querye="UPDATE  `registration` SET `regno`='$regno',`firstName`='$fname',`middleName`='$mname',`lastName`='$lname',
// 		`gender`='$gender',`contactno`='$contactno',`updationDate`='$udate' WHERE `email`='$email'";
...
// 		}
// 		else{
// 			echo"<script>alert('Profile updated Unuccssfull');</script>";
// 		}
// 	}
}
?>
