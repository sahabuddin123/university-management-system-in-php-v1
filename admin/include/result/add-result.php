...
$mark=$_POST['marks'];
 ...
 $stmt = $dbh->prepare("SELECT tblsubjects.SubjectName,tblsubjects.id,tblsubjects.SubjectCradite FROM tblsubjectcombination join  tblsubjects on  tblsubjects.id=tblsubjectcombination.SubjectId WHERE tblsubjectcombination.ClassId=:cid order by tblsubjects.SubjectName");
...
array_push($sid1,$row['id']);
	 array_push($scr,$row['SubjectCradite']);
   } 
...
for($i=0;$i<count($mark);$i++){
	...
	$scr1=$scr[$i];
		...
		if($scr1==6)
		{
		 $s=resultWithCr($mar,220);
		}
	elseif($scr1==4)
		{
		 $s=resultWithCr($mar,160);
		}
	elseif($scr1==3)
		{
		$s=resultWithCr($mar,120);
		}
	elseif($scr1==2)
		{
		$s=resultWithCr($mar,80);
		}
	else 
		{
		$s=resultWithCr($mar,40);
		}
	$gpa=$s;
$sql="INSERT INTO  tblresult(StudentId,ClassId,SubjectId,marks,gpa) VALUES(:studentid,:class,:sid,:marks,:gpa)";
